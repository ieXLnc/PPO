...
class AgentBatch:
    def __init__(self, name, training_old):
        ...
        self.name = './Models_ppo/PPO_batch_' + str(name)
        ...
        if training_old:
            ...
            print('... Load Actor/Critic ppo_batch from previous training ...')
        else:
            print('New Actor/Critique models created from ppo_batch...')
    ...
    def learn(self, learning_steps):
        ...
        while step < learning_steps:
            ...
            for explore_step in range(self.exploration_steps):

                # play for explore steps
            ...
            batch_discounted_reward = torch.tensor(batch_discounted_reward, dtype=torch.float)
            ...
            # do batches
            starts = np.arange(0, self.exploration_steps, self.mini_batch_size)
            rand_idx = np.arange(0, self.exploration_steps)
            np.random.shuffle(rand_idx)
            batches = [rand_idx[i:i+self.mini_batch_size] for i in starts]
            # print('batches', batches)
            # get batches
            batch_log_probs = batch_log_probs.unsqueeze(1)
            batch_discounted_reward = batch_discounted_reward.unsqueeze(1)
            advantage = advantage.unsqueeze(1)

            for batch in batches:

                mini_states = batch_states[batch]
                mini_acts = batch_actions[batch]
                mini_log_probs = batch_log_probs[batch].squeeze()
                mini_discounted = batch_discounted_reward[batch].squeeze()
                mini_advantages = advantage[batch].squeeze()

                for _ in range(self.update_model):

                    batch_new_probs, new_values, entropy = self.evaluate(mini_states, mini_acts)

                    ratios = torch.exp(batch_new_probs - mini_log_probs)
                    surr1 = ratios * mini_advantages
                    surr2 = torch.clamp(ratios, 1 - self.clip, 1 + self.clip) * mini_advantages

                    actor_loss = (- torch.min(surr1, surr2)).mean()
                    critic_loss = nn.MSELoss()(new_values, mini_discounted)
    ...
    def summary(self):
        ...
        print(f"Average Episodic Lenght: {self.exploration_steps} with mini batch {self.mini_batch_size}")
